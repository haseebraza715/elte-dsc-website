...
export default function About() {
  ...
  return (
    <section id="about" className="relative border-b border-border py-20 bg-gradient-to-b from-white via-sky-50/50 to-white overflow-hidden">
      ...
      <div className="relative mx-auto max-w-content px-4 grid gap-12 lg:grid-cols-2 items-start">
        <div>
          ...
          <h2 className="mt-3 text-3xl md:text-4xl font-semibold text-textmain">{content.heading}</h2>
          <div className="mt-4 space-y-4">
            {content.paragraphs.map((p) => (
              <p key={p} className="text-slate-700 leading-relaxed text-[1.05rem]">{p}</p>
            ))}
          </div>
          <div className="mt-6 flex items-center gap-3 text-sky-800">
            <a className="inline-flex items-center gap-2 rounded-md border border-sky-200 bg-white px-4 py-2 text-sm font-medium text-sky-900 hover:bg-sky-50 shadow-sm" href="#contact">
            ...
            <a className="inline-flex items-center gap-2 rounded-md border border-sky-200 bg-white px-4 py-2 text-sm font-medium text-sky-900 hover:bg-sky-50 shadow-sm" href="https://www.linkedin.com" target="_blank" rel="noopener noreferrer">
        ...
        <div className="rounded-2xl bg-white/70 backdrop-blur-sm text-textmain border border-sky-200 p-6 md:p-8 shadow-xl">
          <h3 className="text-xl font-semibold mb-4 text-textmain">Focus Areas</h3>
          <ul className="divide-y divide-sky-100">
            {focus.map((item) => (
              <li key={item} className="py-3 flex items-start gap-3">
                <span className="mt-1 inline-flex items-center justify-center rounded-full bg-sky-600 text-white w-5 h-5">
