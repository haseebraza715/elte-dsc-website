...
const MemberCard = memo(function MemberCard({ person, index }) {
  ...
  const getObjectPosition = () => {
    if (person.name === 'Haseeb Raza') {
      return 'center 20%' // Crop to show upper body, cutting out the hand
    }
    return 'center top'
  }
  ...
  return (
    ...
    >
      <div className="bg-white rounded-xl md:rounded-2xl p-4 sm:p-5 md:p-6 h-full flex flex-col border border-slate-200/60 shadow-sm hover:shadow-xl hover:shadow-sky-500/10 transition-all duration-200 hover:-translate-y-1 md:hover:-translate-y-2 overflow-hidden">
        ...
        <div className="flex flex-col items-center text-center relative z-10">
          ...
          <div className="relative mb-4 sm:mb-5 w-full">
            <div className="relative w-full aspect-[3/4] rounded-lg md:rounded-xl overflow-hidden bg-gradient-to-br from-sky-500 to-blue-600 shadow-md md:shadow-lg group-hover:shadow-xl group-hover:scale-[1.01] md:group-hover:scale-[1.02] transition-all duration-200">
              {imageSrc && !imageError ? (
                <>
                  {/* Fallback initials shown behind image */}
                  <div className="absolute inset-0 bg-gradient-to-br from-sky-500 to-blue-600 flex items-center justify-center text-white text-xl md:text-2xl font-bold z-0">
          ...
          {/* Name */}
          <h3 className="text-lg sm:text-xl font-bold text-slate-900 mb-2 group-hover:text-sky-600 transition-colors duration-200">
            {person.name}
          </h3>
          
          {/* Role Badge */}
          <div className="mb-3 sm:mb-4">
            <span className="inline-flex items-center px-3 sm:px-4 py-1 sm:py-1.5 rounded-full text-xs font-semibold uppercase tracking-wide bg-gradient-to-r from-sky-100 to-blue-100 text-sky-700 border border-sky-200/60 shadow-sm">
              {person.role}
            </span>
          </div>
          ...
          {/* Meta Info */}
          {person.meta && (
            <div className="flex items-center justify-center mb-4 sm:mb-5 text-slate-600">
              <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 mr-1.5 sm:mr-2 text-sky-500 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" />
              </svg>
              <span className="text-xs sm:text-sm font-medium">{person.meta}</span>
            </div>
          )}
          ...
          {person.link && (
            <div className="mt-auto w-full">
              <a
                ...
                className="group/link w-full inline-flex items-center justify-center px-4 sm:px-5 py-2 sm:py-2.5 rounded-lg sm:rounded-xl text-xs sm:text-sm font-semibold text-white bg-gradient-to-r from-sky-600 to-blue-600 hover:from-sky-700 hover:to-blue-700 active:from-sky-800 active:to-blue-800 border-2 border-transparent hover:border-sky-500/30 transition-all duration-200 shadow-md hover:shadow-lg hover:shadow-sky-500/30 active:scale-95"
              >
                <svg className="w-3.5 h-3.5 sm:w-4 sm:h-4 mr-1.5 sm:mr-2 transition-transform duration-200 group-hover/link:scale-110 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24">
...
export default function Members() {
  return (
    <section id="members" className="relative bg-gradient-to-br from-slate-50 via-white to-sky-50/60 overflow-hidden">
      ...
      <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 pb-16 sm:pb-20 md:pb-24">
        <div className="grid gap-6 sm:gap-8 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 relative z-10">
